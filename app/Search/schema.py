...
class SearchQuery(graphene.ObjectType):   
    '''Search Query, fetch person entries matching to provided criteria'''

    search = graphene.Field(SearchResultType, query=graphene.NonNull(graphene.String), limit=graphene.Int(10))
    # search_advanced = graphene.Field(SearchResultType, query=graphene.Argument(SearchType))
    ...
    def resolve_search_advanced(self, info, **args):
        query = args.get("query")
        q, l, s = query.get("query"), query.get("first"), query.get("skip")
        result = service.filter(query=q,limit=l,skip=s)
...
schema = graphene.Schema(query=SearchQuery, mutation=None, auto_camelcase=True, types=[SearchResultType, PersonType])
